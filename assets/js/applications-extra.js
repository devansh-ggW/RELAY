(function(){
  ...
  const render=async()=>{
    ...
    root.querySelectorAll('.applicant-card').forEach((card,i)=>{
      ...
      card.querySelector('button[data-app]')?.addEventListener('click',async()=>{try{await RelayDB.createConversation({seeker_id:a.applicant_id,employer_id:s.user.id,job_id:a.job_id});location.href='messages.html'}catch(err){if(err.message?.toLowerCase().includes('duplicate'))location.href='messages.html';else toast(err.message||'Could not start conversation.')}})
    });
  };

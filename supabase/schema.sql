-- Relay production starter schema for Supabase.
create extension if not exists pgcrypto;

do $$ begin create type public.user_role as enum ('seeker','employer'); exception when duplicate_object then null; end $$;
do $$ begin create type public.job_type as enum ('Full-time','Part-time','Freelance','Contract'); exception when duplicate_object then null; end $$;

create table if not exists public.profiles (
  id uuid primary key references auth.users(id) on delete cascade,
  role public.user_role not null default 'seeker', name text not null, email text,
  city text, state text, about text, headline text, skills text[] not null default '{}', phone text,
  date_of_birth date, age integer, age_confirmed boolean not null default false,
  terms_accepted_at timestamptz, privacy_accepted_at timestamptz, onboarding_complete boolean not null default false,
  experience_years integer, avatar_url text, created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);

create table if not exists public.jobs (
  id uuid primary key default gen_random_uuid(), owner_id uuid not null references public.profiles(id) on delete cascade,
  title text not null, company text not null, category text not null, location text not null, state text,
  mode text not null check (mode in ('Remote','Hybrid','On-site')), type public.job_type not null,
  salary text not null, skills text[] not null default '{}', description text not null, application_url text,
  owner_name text, min_age integer default 18, status text not null default 'open' check (status in ('open','closed')),
...
create table if not exists public.applications (
  ...
  cover_note text, created_at timestamptz not null default now(), unique(job_id, applicant_id)
);
...
create policy jobs_select_open on public.jobs for select to anon, authenticated using (status = 'open' or owner_id = auth.uid());
drop policy if exists jobs_insert_employer on public.jobs;
create policy jobs_insert_employer on public.jobs for insert to authenticated with check (owner_id = auth.uid() and exists (select 1 from public.profiles p where p.id = auth.uid() and p.role = 'employer' and p.age_confirmed = true and p.onboarding_complete = true));
...
create policy applications_select_participants on public.applications for select to authenticated using (applicant_id = auth.uid() or exists (select 1 from public.jobs j where j.id = job_id and j.owner_id = auth.uid()));
drop policy if exists applications_insert_self on public.applications;
create policy applications_insert_self on public.applications for insert to authenticated with check (applicant_id = auth.uid() and exists (select 1 from public.jobs j join public.profiles p on p.id = auth.uid() where j.id = job_id and j.status = 'open' and p.age_confirmed = true and p.onboarding_complete = true));
...
begin
  insert into public.profiles (id, role, name, email) values (new.id, case when new.raw_user_meta_data->>'role' = 'employer' then 'employer'::public.user_role else 'seeker'::public.user_role end, coalesce(new.raw_user_meta_data->>'name', split_part(coalesce(new.email,''),'@',1), 'Relay user'), new.email) on conflict (id) do nothing;
  return new;
end;
$$;
